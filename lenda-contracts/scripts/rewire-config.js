...
async function main() {
    console.log("🛠️  Rewiring GoldfinchConfig with CORRECT indices...\n");
    ...
    for (const item of mapping) {
        ...
        // We bypass getAddress check because we want to FORCE initialize the correct ones 
        // if they are zero. But setAddress has a check if addresses[index] == 0.
        // If current is NOT zero and NOT the target, we have a problem (it's immutable once set).
        // But in my case, indices 0, 1, 2, 4, 5, 12, 13, 14 etc are probably zero.

        const current = await config.getAddress(item.index);
        console.log(`🔍 Checking ${item.name} [${item.index}]: Current=${current}, Target=${item.addr}`);

        if (current.toLowerCase() === item.addr.toLowerCase()) {
            console.log(`   ✅ Already correct.`);
            continue;
        }

        if (current !== "0x0000000000000000000000000000000000000000") {
            console.log(`   ⚠️  Index already set to ${current}. Cannot change to ${item.addr} (Immutable)`);
            continue;
        }

        console.log(`   📡 Setting ${item.name} at [${item.index}] to ${item.addr}...`);
        try {
            const tx = await config.setAddress(item.index, item.addr);
            await tx.wait();
            console.log("   ✅ Done.");
        } catch (e) {
